...
foreach ($array as $key=>$value) {  
   ...
   // $photo = getImage($photo);
    ...
    $data[$key]['upc'] = (isset($value['UPC CODE']) && !empty($value['UPC CODE']) && $value['UPC CODE'] != ' ')?$value['UPC CODE']:'';
...
function saveProducts($array,$obj)
{
    ...
    if(isset($productId) && !empty($productId))
    {
        ...
        $tableName = $resource->getTableName('mgto_marketplace_product'); //gives table name with prefix
        ...
        if(is_array($result) && !empty($result))
        {
            ...
            $seller_id = 7; //need to change according to selected seller.
            $qty = (isset($array['qty']) && !empty($array['qty']) && $array['qty'] != ' ')?$array['qty']:''; 
            $price = (isset($array['price']) && !empty($array['price']) && $array['price'] != ' ')?$array['price']:''; 
            $description = (isset($array['description']) && !empty($array['description']) && $array['description'] != ' ')?trim($array['description']):''; 
            $condition = 1;
            $type = (isset($array['product_type']) && !empty($array['product_type']) && $array['product_type'] != ' ')?$array['product_type']:''; 
            $created_at = date('Y-m-d H:i:s');
            $status = 1;
            $themeTable = $resource->getTableName('mgto_marketplace_assignproduct_items');
            $sql = "INSERT INTO " . $themeTable . "(product_id, owner_id, seller_id,qty,price,description,options,image,$themeTable.condition,type,created_at,status) VALUES ('".$productId."','".$owner_id."','".$seller_id."','".$qty."','".$price."','".$description."','','','".$condition."','".$type."','".$created_at."','".$status."')"; 
            $response = $connection->query($sql);
            ...
            if(isset($lastInsertId) && !empty($lastInsertId))
            {
                ...
                {
                    $type = 2;
                    $assign_id = $lastInsertId;
                    $value = $description;
                    $is_default = 1;
                    $status = 1;
                    $store_view = 1;
                    $themeTable1 = $resource->getTableName('mgto_marketplace_assignproduct_data');
                    $sql = "INSERT INTO " . $themeTable1 . "(type, assign_id, value,date,is_default,status,store_view) VALUES ('".$type."','".$assign_id."','".$value."','','".$is_default."','".$status."','".$store_view."')"; 
                    $response = $connection->query($sql);
                    $lastInsertId1 = $connection->lastInsertId();
                }
   ...
   // $product->setCustomAttribute(149,$catalogPage);
    ...
    if($productId)
    {
    	$resources = \Magento\Framework\App\ObjectManager::getInstance()
			...
			$seller_id = 9; //need to update on live
